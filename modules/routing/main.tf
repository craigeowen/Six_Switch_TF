...
##### Static Routes #####
resource "nxos_ipv4_static_route" "SR-for-bgp-agg01" {
  provider = nxos.twe-agg01
  for_each = {for k, v in var.static-routes_map : k => v} 
  vrf_name = "${each.value.vrf_name}"
  prefix   = "${each.value.prefix-agg02}"
  next_hops = [{
    interface_id = "${each.value.interface_id}"
    address      = "${each.value.nh-address-agg01-agg02}"
    vrf_name     = "${each.value.vrf_name}"
  }]
}

resource "nxos_ipv4_static_route" "SR-for-bgp-agg02" {
  ...
  next_hops = [{
    interface_id = "${each.value.interface_id}"
    address      = "${each.value.nh-address-agg02-agg01}"
    vrf_name     = "${each.value.vrf_name}"
  }]
}
