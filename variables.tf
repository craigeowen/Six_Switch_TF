...
variable "po_sub-int-vrf_map-agg01" {
  type = map(object({
    interface_id = string
    vrf_dn = string
}))
}
variable "po_sub-int-vrf_map-agg02" {
  type = map(object({
    interface_id = string
    vrf_dn = string
}))
